...
#' @description Leave-one-out method gives approximately independent sample of standard multivariate normal distribution,
#' which then produces sample of standard univariate normal distribution.
#'
#' @param x multivariate data matrix
#'
#' @return Data frame containning univariate data and the index from multivariate data.
#'
#' @details
#' \deqn{S_{-k}^{-1/2} (X_k - \bar{X}_{-k}) , k = 1,... n}
#' are approximately independent sample of \eqn{N_p(0, I)}, which then gives \eqn{n \times p}
#' univariate sample of \eqn{N(0, 1)}.
# @note The transformation gives number of data points as a multiple of dimension \eqn{p}, density of data points is high and
# spline method may produce ill-posed matrices. Merging with \code{x.dist} parameter is recommended.
#'
#'
#' @export
#' @examples
