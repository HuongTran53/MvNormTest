...
#' @description
#' Taylor expansion implies that vectors of derivatives of \eqn{\log(\hat{M}_X(t))} can be approximated
#' by a linear combination of vectors of derivatives of \eqn{\hat{M}_X(t)}. \code{matrix_A} results the corresponding
#' linear combinations.
#'
#' @param t vector of \eqn{\mathbb{R}^p}
#'
#' @return \code{mt3_matrix_A} returns coefficient matrix relating to the use of third derivatives.
#'
#'
#'
#' @examples
...
############################################
#' @rdname matrix_A
#' @return \code{mt4_matrix_A} returns coefficient matrix relating to the use of fourth derivatives.
#' @export
#'
mt4_matrix_A <- function(t){
  ...
  for (j1 in 1: p){
    indT4 <- which(apply(pos.matrixT4, 1, function(r) all(r == rep(j1, 4))))
    vA0001 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, 0, j1))))
    vA0011 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j1, j1))))
    vA0111 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j1, j1, j1))))
    vA1111 <- which(apply(pos.matrix, 1, function(r) all(r == c(j1, j1, j1, j1))))
    ...
    for (j2 in j1:p){
      #########################
      vA0002 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, 0, j2))))

      vA0012 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j1, j2))))
      vA0022 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j2, j2))))


      vA0112 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j1, j1, j2))))
      vA0122 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j1, j2, j2))))
      vA0222 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j2, j2, j2))))


      vA1112 <- which(apply(pos.matrix, 1, function(r) all(r == c(j1, j1, j1, j2))))
      vA1222 <- which(apply(pos.matrix, 1, function(r) all(r == c(j1, j2, j2, j2))))
      vA1122 <- which(apply(pos.matrix, 1, function(r) all(r == c(j1, j1, j2, j2))))

      if (j2 > j1){
        #########################
        # when c(j1, j1, j1, j2)
        indT4 <- which(apply(pos.matrixT4, 1, function(r) all(r == c(j1, j1, j1, j2))))
        ...
        #########################
        # when c(j1, j2, j2, j2)
        indT4 <- which(apply(pos.matrixT4, 1, function(r) all(r == c(j1, j2, j2, j2))))
        ...
        #########################
        # when c(j1, j1, j2, j2)
        indT4 <- which(apply(pos.matrixT4, 1, function(r) all(r == c(j1, j1, j2, j2))))
      ...
      for (j3 in j2:p){
        vA0003 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, 0, j3))))

        vA0013 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j1, j3))))
        vA0023 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j2, j3))))
        vA0033 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j3, j3))))


        vA0113 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j1, j1, j3))))
        vA0123 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j1, j2, j3))))
        vA0133 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j1, j3, j3))))
        vA0223 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j2, j2, j3))))
        vA0233 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j2, j3, j3))))

        vA1123 <- which(apply(pos.matrix, 1, function(r) all(r == c(j1, j1, j2, j3))))
        vA1223 <- which(apply(pos.matrix, 1, function(r) all(r == c(j1, j2, j2, j3))))
        vA1233 <- which(apply(pos.matrix, 1, function(r) all(r == c(j1, j2, j3, j3))))

        if ((j3 > j2)&(j2 > j1)){
          #########################
          # when c(j1, j1, j2, j3)
          indT4 <- which(apply(pos.matrixT4, 1, function(r) all(r == c(j1, j1, j2, j3))))
          ...
          #########################
          # when c(j1, j2, j2, j3)
          indT4 <- which(apply(pos.matrixT4, 1, function(r) all(r == c(j1, j2, j2, j3))))
          ...
          #########################
          # when c(j1, j2, j3, j3)
          indT4 <- which(apply(pos.matrixT4, 1, function(r) all(r == c(j1, j2, j3, j3))))
        ...
        for (j4 in j3:p){
          vA0004 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, 0, j4))))

          vA0014 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j1, j4))))
          vA0024 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j2, j4))))
          vA0024 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j3, j4))))
          vA0034 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, 0, j3, j4))))

          vA0124 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j1, j2, j4))))
          vA0134 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j1, j3, j4))))
          vA0234 <- which(apply(pos.matrix, 1, function(r) all(r == c(0, j2, j3, j4))))

          vA1234 <- which(apply(pos.matrix, 1, function(r) all(r == c(j1, j2, j3, j4))))
          # When j1 < j2 < j3 < j4
          if (all(!duplicated(c(j1, j2, j3, j4)))){
            indT4 <- which(apply(pos.matrixT4, 1, function(r) all(r == c(j1, j2, j3, j4))))
