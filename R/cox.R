...
#'
#' @description Score function of a univariate normal distribution is a straight line.
#' A non-linear graph of score function estimator shows evidence of non-normality.
#'
...
#' \item{\code{slt}: Index of merged data point (is \code{NULL} if \code{x.dist = NULL}).}
#' }
#'
#' @details
#' To avoid the singularity of coefficient matrices in spline method, points with distance less than \code{x.dist}
#' are merged and weight of the representative points is updated by the summation of weight of discarded points.
#'
#' @export
#' @import MatrixExtra
#' @import Matrix
...
#' @references
#' \insertRef{ref:sp_estimate}{MvNormTest}
#'
#' @examples
#' set.seed(1)
#' x <- rnorm(100, 2, 4)
...
cox <- function(x, P = NULL, lambda = 0.5, x.dist = NULL) {
  ...
  Q <- Matrix::bandSparse(nx, nxm2, -2: 0, list(1 / h[-1],
                                                ...
                                                1 / h[-nxm1]))
  # Matrix P^-1Q is pq:
  pq <- Matrix::bandSparse(nx, nxm2, -2:0, list(1/(P[-c(1,2)] * h[-1]),
  ...
  #Matrix R is r:
  r <- Matrix::bandSparse(nxm2, nxm2, (-1): 1, list(h[-c(1, length(h))] / 3,
                                                    2 / 3 *(h[-1] + h[-length(h)]),
                                                    h[-c(1, length(h))] / 3))
  #Pseudo y:
  ...
  invtemp <- solve(temp)
  #compute Q'y:
  qy <- suy[-c(nxm1, nx)] * (1/h[-nxm1]) - suy[-c(1,nx)] *(1/h[-nxm1] + 1/h[-1]) + suy[-c(1,2)] * (1/h[-1])
  #compute a:
