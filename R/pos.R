...
# @description
# Given the position in the chain containg all derivatives of estimator \eqn{\hat{M}_X(t)}, get the information
...
#' @param p Dimension
#'
#' @return \code{mt3_rev_pos} returns the position of this particular derivative in the chain of all derivatives, up to third order.
#'
#' @details
...
#' \deqn{
#' Z = \bigg(\hat{M}, \hat{M}^{001}, \dots \hat{M}^{00p}, \hat{M}^{011}, \hat{M}^{012}, \dots \hat{M}^{0pp}, \hat{M}^{111},
...
#' @rdname CGF_transformations
#' @description
#' Given dimension \eqn{p}, returns a dataframe containing the position of all derivatives of
#' estimator of moment generating function \eqn{\hat{M}_X(t)}, upto third/fourth order.
...
##################################
#' @rdname CGF_transformations
#' @return \code{mt4_pos} an array contaning all position with respect to index of \eqn{j_1, j_2, j_3, j_4}.
#' @export
